...
#define ID_MQTT  "lens_Ar5FqZmhWgZOgO5RgT7NGkZTp9P"     /* id mqtt (para identificação de sessão)
...
void envia_informacoes_por_mqtt(float temp_lida, float umid_lida)
{
  ...
  //constrói mensagens e as envia
  //- temperatura atual
  sprintf(mensagem_MQTT, "Temperatura: %.2f C, Umidade atual: %.2f%, Temperatura maxima: %.2f C, Temperatura minima: %.2f C", temp_lida, umid_lida, temperatura_max, temperatura_min);
  MQTT.publish(TOPICO_PUBLISH, mensagem_MQTT);

  //sprintf(mensagem_MQTT, "Temperatura: %.2f °C", temp_lida);
  //MQTT.publish("/paic/uea/danilo/temp", mensagem_MQTT);

  //sprintf(mensagem_MQTT, "Umidade: %.2f%", umid_lida);
  //MQTT.publish("/paic/uea/danilo/umid", mensagem_MQTT);
}
