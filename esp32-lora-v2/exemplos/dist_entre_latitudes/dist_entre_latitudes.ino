...
float lat_maior = 1.0/0.0;
float lon_maior;
float lat_menor = 1.0/0.0;
float lon_menor;
float distancia;
...
void envia_medicoes_serial(TDadosLora dados_lora) 
{
  ...
  if (dados_lora.erro_gps) {
    ...
    memset(mensagem,0,sizeof(mensagem));
    sprintf(mensagem,"Distancia: %.2f", distancia);
    Serial.println(mensagem);
  }
...
void loop()
{
  ...
  if (!newData)
    ...
    if(isinf(lat_maior) || dados_lora.f_latitude > lat_maior){
      lat_maior = dados_lora.f_latitude;
      lon_maior = dados_lora.f_longitude;
    }
    if(isinf(lat_menor) || dados_lora.f_latitude < lat_menor){
      lat_menor = dados_lora.f_latitude;
      lon_menor = dados_lora.f_longitude;
    }

    distancia = gps.distance_between(lat_maior, lon_maior, lat_menor, lon_menor);
  }
